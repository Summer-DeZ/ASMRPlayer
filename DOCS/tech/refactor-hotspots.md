...
- **问题**：语言统一阶段完成，主源码 `@JvmField`、`@JvmStatic`、`@JvmOverloads` 迁移兼容注解和明确 Java 平台值 helper 已清零，但部分迁移期 API 仍保留 Java null 宽松边界，尚未完全发挥 Kotlin 空安全和数据建模能力。
- **方向**：C2 已完成；下一步继续处理迁移期兼容点，按调用链收紧剩余 nullable API。

### C3 · [P2] 接口/model nullable 参数泛滥 + 防御式 `if-null-return`（Java 移植味）

- **位置**：`LibraryRepository` 已完成 Phase F/C3 第一切片，playlist/track/subtitle/cover/selected playlist 相关入参改为非空 Kotlin 类型；`DlsiteRepository`、`DlsiteLocalStore` 与 `DlsiteDownloadQueueRepository` 已完成第二切片，`workId`、`taskId`、`optionId`、`DlsiteWork` 与 `saveWork(updatedWork)` 等 identity/required 入参改为非空 Kotlin 类型；核心资料库模型 `TrackItem` 与 `Playlist` 已删除迁移期 `@JvmOverloads`、字段 `@JvmField` 和 nullable 构造语义，构造参数收紧为非空 Kotlin 类型；DLsite 下载 DTO `DlsiteDownloadOption` 与 `DlsiteJsonParser.ContentCount` / `DlsiteZiptree` / `ContentFile` 已删除字段 `@JvmField` 和 nullable 构造语义，parser 层继续负责空值归一化；DLsite 核心模型 `DlsiteWork` 与 `DlsiteContent` 的下载完成、URL/URI/title 更新、封面推导和发现结果合并等 required 状态转换入参已改为非空，错误详情与选项标题保留可空兜底；`DlsiteDownloadQueueTask` 已删除字段 `@JvmField`，保留 data class 默认参数、copy、JSON/static helper、队列 nullable 状态字段与状态转换语义；`DlsiteDownloadTask` 嵌套结果/进度 DTO、`SubtitleCue`、`PendingLibrarySelection.TrackSubtitleTarget`、`DocumentFiles.FolderImportItem`、`AppThemePalette` 与 `AppUi` 已删除剩余字段 `@JvmField`；主源码剩余 `@JvmStatic` / `@JvmOverloads` 已删除，保留 Kotlin 默认参数、`@Throws`、`const val`、object/companion 结构和 nullable 语义；`SubtitleCue.text`、`DocumentFiles.FolderImportItem.audioName/audioUri/subtitleName` 与 `TreeDocument` 已删除 `preserveJavaFieldValue`、`platformValue` 和 `platformFieldValue` helper，改为非空 Kotlin 构造语义；`DlsiteJsonParser` / `DlsiteLibraryJsonParser` / `DlsiteDownloadJsonParser` / `DlsiteWebvttJsonParser` 的完整 JSON 文本入参、`DlsiteJsonSupport.parse` 和 `JsonReader` 已改为非空 `String`，JSON 字段层面的容错 helper 保留；`DlsiteHtmlParser` 的完整 HTML、页面 URL 和目标 `workId` facade 入参已改为非空，空 HTML/找不到链接/非法 base URL 语义保留；`DlsiteRemoteFiles` 的路径/查询编码、安全文件名、body 摘要和 HTML/JSON 文件探测入参已改为非空，不存在文件返回 false 与删除清理 nullable 兜底保留；`DlsiteCoverRemote` 复用 `DlsiteRemoteFiles` 编码、sniff、清理、摘要和中断工具，重复 helper 已删除，`DlsiteCoverRemote` / `DlsiteWorkRemote` / `DlsiteContentRemote` 私有 HTTP helper 必填参数已改为非空；`DlsiteDownloadPlanner.optionsFor`、`DlsiteDownloadTask.downloadAndImport` 必填协作者、`DownloadProgressTracker`、`Result` / `ContentResult` / `DownloadedContent` / `ImportResult` 内部构造数据已改为非空 Kotlin 类型；`DlsiteClient`、`DlsiteContentRemote`、`DlsiteCoverRemote` 与 `DlsiteRemoteFiles` 的必填 `DlsiteWork`、目标文件、目录和 `DlsiteHttpClient` 注入已改为非空；`DlsiteDownloadBlockingAdapter`、`DlsiteDownloadService.DownloadRequest` 与 `DlsiteDownloadTask.ContentListener` 的 required 下载链路入参已改为非空，Android Intent 边界继续在 Service 层归一化；`rg -n "@JvmField|@JvmStatic|@JvmOverloads" app/src/main/java` 无输出。Room 实现继续保留空字符串业务校验。
- **问题**：Kotlin 侧本可用非空类型在编译期挡住，旧 Java 迁移边界把校验下沉到运行时、每个方法重复；如果继续扩大，会让业务缺失值和 null 兼容输入混在同一层。
- **方向**：剩余 C3 工作转入其它 model/facade nullable helper；继续按调用链收紧真实 required 参数；`startedAt`、`finishedAt`、`errorMessage`、`optionTitle`、`subtitleUri`、底层 `DlsiteHttpClient` 容错参数、下载内容删除入口 `optionId` 与 Android/系统回调继续保留 nullable；历史入口继续在 Service、intent helper 或 facade 内部归一化。
