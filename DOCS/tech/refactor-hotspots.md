...
### C2 · [P1] 11 个 Java 文件未完成 Kotlin 化（Java 11 / Kotlin 94，约 10%）

- **位置**：核心 model、`data/remote` 和 `data/files/DocumentFiles` 已迁移到 Kotlin 并保留 Java 下载服务/Task 所需 ABI；剩余 Java 集中在 `data/download/`（3 个）、`ui/theme/`（`AppUi/AppThemePalette/AppThemeMode`）、`ui/activity/DlsiteLoginActivity.java`、`playback/`（`SubtitleParser/SubtitleCue/SubtitleOverlayWindow/PlaybackIntents`）。
- **问题**：核心 model、远程层和文件工具的 Java 残留已解决，但下载链路、字幕/窗口工具和旧 UI/theme 类仍让 Kotlin 调用失去完整空安全与数据建模能力。
- **方向**：下一步按模块边界迁 `data/download/`，再处理字幕、悬浮窗和旧 UI/theme 工具类；nullable API 收紧应在对应 Java 调用点迁完后单独落地。
