...
- [x] 需要做：Phase C/D1 快照压缩小切片：`PlaybackServiceSnapshot` / `sessionExtras` 删除未消费的 `previousSubtitle`、`currentSubtitle`、`nextSubtitle` 邻接字幕文本，悬浮窗仍由 `PlaybackService.lastSubtitleText` 驱动。
